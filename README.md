...
🤔 Decision Making Agent - Analyzes queries and makes strategic decisions
✅ Evaluator Tool - Integrated tool that assesses recommendation quality
🏗️ Architecture
image/png
...
How It Works
When a user sends a message, the system processes it through the multi-agent framework
As tools are called (customer data access, web search, evaluation), the UI shows real-time status
Response timer tracks total elapsed time for each interaction
Users can see exactly what the agent is doing and how long it takes
Each browser session maintains its own tool status and timer independently
...
🔄 Workflow
User Query → CSM asks a question about a customer
Response Timer Starts → Begins tracking elapsed time
Decision Agent → Analyzes the query and determines approach
Data Gathering → Accesses customer data and web research (if high-risk)
Decision Agent → Generates detailed recommendations first
Evaluator Tool → Assesses recommendations with minimal JSON response ({"rating": 8, "pass": true})
Quality Gate → If pass: false, automatically retries with feedback (up to 2 retries)
Response Timer Stops → Records final elapsed time
Result → Returns clean recommendations without evaluator details
🎯 Key Features
